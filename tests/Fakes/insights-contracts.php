 ...
 *
 * Required once from `tests/Pest.php`, before any test runs, which is the same
 * trade `tests/Fixtures/StandInActivityFacade.php` makes for the other optional
 * sibling and for the same reason: `class_exists()` then gives every test in the
 * run the same answer, instead of one that depends on file order.
 *
 ...
 * it.
 *
 * The base class the metrics extend is not here but in
 * `insights-table-metric.php`, which is a verbatim copy rather than a rewrite.
 * See that test for why a base class has to be held to a stricter standard than
 * an interface.
 */
